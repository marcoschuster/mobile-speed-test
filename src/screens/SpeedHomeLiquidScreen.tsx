...
import VideoQualityCard, { VideoStreamingAssessment } from '../components/VideoQualityCard';
...
import VideoStreamingTest, { HiddenVideoStreamingTestHost } from '../services/VideoStreamingTest';
...
const PERFORMANCE_WARNING_SECONDS = 30 * 60;
const HEAT_WARNING_SECONDS = 15 * 60;
...
const getGamingGradeColor = (grade: string, t: ReturnType<typeof useTheme>['t']) => {
...
const getIntervalWarning = (totalSeconds: number) => {
  if (totalSeconds > 0 && totalSeconds < HEAT_WARNING_SECONDS) {
    return 'Intervals under 15 minutes may reduce phone performance and heat up the device.';
  }

  if (totalSeconds > 0 && totalSeconds < PERFORMANCE_WARNING_SECONDS) {
    return 'Intervals under 30 minutes may reduce phone performance.';
  }
...
const GamingIcon = ({ size = 20, color = '#FFFFFF' }: { size?: number; color?: string }) => (

  <Svg width={size} height={size} viewBox="0 0 24 24">

    <Path

      d="M7 10h10a4 4 0 0 1 3.8 5.2l-1 3A3 3 0 0 1 17 20h-1.2a2 2 0 0 1-1.6-.8l-1-1.3a1.5 1.5 0 0 0-2.4 0l-1 1.3A2 2 0 0 1 8.2 20H7a3 3 0 0 1-2.8-1.8l-1-3A4 4 0 0 1 7 10Zm2 2v2H7v2h2v2h2v-2h2v-2h-2v-2H9Zm7 2.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2Zm2 3a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z"
...
const VideoStreamIcon = ({ size = 20, color = '#FFFFFF' }: { size?: number; color?: string }) => (

  <Svg width={size} height={size} viewBox="0 0 24 24">

    <Path

      d="M4 7.5A2.5 2.5 0 0 1 6.5 5h7A2.5 2.5 0 0 1 16 7.5v9a2.5 2.5 0 0 1-2.5 2.5h-7A2.5 2.5 0 0 1 4 16.5v-9Zm12 2.2 3.2-2.05A1.2 1.2 0 0 1 21 8.67v6.66a1.2 1.2 0 0 1-1.8 1.02L16 14.3V9.7Z"
...
const SpeedHomeLiquidScreen = () => {
  ...
  const [isGamingTestRunning, setIsGamingTestRunning] = useState(false);
  const [gamingResult, setGamingResult] = useState<GamingResult | null>(null);
  const [isVideoStreamingTestRunning, setIsVideoStreamingTestRunning] = useState(false);
  const [videoStreamingResult, setVideoStreamingResult] = useState<VideoStreamingAssessment | null>(null);
  ...
  const loadPersistedData = useCallback(async () => {
    ...
    try {
      ...
      const storedHistory = await SpeedTestService.getHistory();

      const history = Array.isArray(storedHistory) ? storedHistory.filter(Boolean) : [];
      ...
      console.warn('Speed tab failed to restore persisted data:', error);

      setHistorySummary(summarizeHistory([]));
      ...
      setPeaks({ download: 0, upload: 0, ping: 0 });
  ...
  const startGamingTest = useCallback(async () => {

    if (isTestRunning || isGamingTestRunning || isVideoStreamingTestRunning) {

      return;

    }

    setIsGamingTestRunning(true);

    setProgressText('Running gaming test...');

    try {

      const GamingModeTest = require('../services/GamingModeTest').default;

      const result = await GamingModeTest.run(({ elapsedMs }: { elapsedMs: number }) => {

        const remaining = Math.max(0, Math.ceil((10000 - elapsedMs) / 1000));

        setProgressText(`Gaming test... ${remaining}s`);

      });

      setGamingResult(result);

      setProgressText(`Gaming grade: ${result.grade}`);

      SoundEngine.playTestComplete();

    } catch (error: any) {

      Alert.alert('Gaming Test Failed', error?.message || 'Unable to complete gaming mode diagnostics.');

      setProgressText('');

    } finally {

      setIsGamingTestRunning(false);

    }

  }, [isGamingTestRunning, isTestRunning, isVideoStreamingTestRunning]);

  const startVideoStreamingTest = useCallback(async () => {
    ...
    const measuredDownloadMbps = downloadSpeed || lastTest?.download || 0;

    if (measuredDownloadMbps <= 0) {

      Alert.alert('Run a speed test first', 'Measure the connection before running the streaming probe.');

      return;

    }

    setIsVideoStreamingTestRunning(true);
    setProgressText('Starting video streaming probe...');

    try {

      const result = await (VideoStreamingTest as any).run({
        measuredDownloadMbps,
        onProgress: (message: string) => setProgressText(message),
      });

      setVideoStreamingResult(result);
      setProgressText(result.summary);
      SoundEngine.playTestComplete();

    } catch (error: any) {

      Alert.alert('Video Test Failed', error?.message || 'Unable to complete the streaming probe.');
      setProgressText('');

    } finally {

      setIsVideoStreamingTestRunning(false);
      ...
      >
        ...
        {!isTestRunning ? (
          ...
          <LiquidGlass
            ...
            onPress={startGamingTest}

            style={styles.gamingButton}

            contentStyle={styles.gamingButtonContent}
          ...
          >
            ...
            <GamingIcon color={t.textPrimary} />

            <Text style={[styles.gamingButtonText, { color: t.textPrimary }]}>

              {isGamingTestRunning ? 'Gaming Test Running...' : 'Gaming Test'}
        ...
        {!isTestRunning ? (
          ...
          <LiquidGlass
            ...
            onPress={startVideoStreamingTest}
          ...
          >
            ...
            <VideoStreamIcon color={t.textPrimary} />

            <Text style={[styles.gamingButtonText, { color: t.textPrimary }]}>

              {isVideoStreamingTestRunning ? 'Video Test Running...' : 'Video Streaming Test'}
        ...
        {gamingResult ? (
          ...
          <View style={styles.gamingCardWrap}>

            <LiquidGlass contentStyle={styles.gamingResultContent}>

              <View style={styles.gamingResultHeader}>

                <Text style={[styles.gamingResultTitle, { color: t.textMuted }]}>Gaming Readiness</Text>

                <View

                  style={[

                    styles.gamingGradePill,

                    {

                      backgroundColor: withAlpha(getGamingGradeColor(gamingResult.grade, t), 0.16),

                      borderColor: withAlpha(getGamingGradeColor(gamingResult.grade, t), 0.34),
                ...
                >
                  ...
                  <Text style={[styles.gamingGradeText, { color: getGamingGradeColor(gamingResult.grade, t) }]}>
              ...
              <Text style={[styles.gamingGames, { color: t.textPrimary }]}>{gamingResult.games}</Text>

              <Text style={[styles.gamingSummary, { color: t.textSecondary }]}>{gamingResult.summary}</Text>

              <View style={styles.gamingMetricsRow}>

                <View style={styles.gamingMetric}>

                  <Text style={[styles.gamingMetricLabel, { color: t.textMuted }]}>Avg</Text>

                  <Text style={[styles.gamingMetricValue, { color: t.textPrimary }]}>{gamingResult.avgLatency.toFixed(1)}ms</Text>

                </View>

                <View style={styles.gamingMetric}>

                  <Text style={[styles.gamingMetricLabel, { color: t.textMuted }]}>Jitter</Text>

                  <Text style={[styles.gamingMetricValue, { color: t.textPrimary }]}>{gamingResult.jitter.toFixed(1)}ms</Text>

                </View>

                <View style={styles.gamingMetric}>

                  <Text style={[styles.gamingMetricLabel, { color: t.textMuted }]}>Loss</Text>

                  <Text style={[styles.gamingMetricValue, { color: t.textPrimary }]}>{gamingResult.packetLoss.toFixed(2)}%</Text>

                </View>

                <View style={styles.gamingMetric}>

                  <Text style={[styles.gamingMetricLabel, { color: t.textMuted }]}>Spike</Text>

                  <Text style={[styles.gamingMetricValue, { color: t.textPrimary }]}>{gamingResult.maxLatencySpike.toFixed(1)}ms</Text>
        ...
        {videoStreamingResult ? (
          ...
          <View style={styles.videoCardWrap}>
            ...
            <VideoQualityCard result={videoStreamingResult} />
              ...
              <LiquidGlass style={styles.intervalPanel} contentStyle={styles.intervalPanelContent}>
                ...
                {customIntervalOpen ? (
                  ...
                  <View style={styles.customIntervalWrap}>
                    ...
                    {getIntervalWarning(customIntervalSeconds) ? (
